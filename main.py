...
def parse_vk_input(text):
    ...
    # 2. Поддержка формата ручного ввода ID и Токена (если передан полный формат)
    if len(lines) >= 3 and (lines[0].startswith("-100") or lines[0].startswith("@")) and ":" in lines[1]:
        return {
            "action": "post_custom",
            "chat_id": lines[0],
            "token": lines[1],
